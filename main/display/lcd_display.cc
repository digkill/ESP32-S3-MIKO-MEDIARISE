...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    ESP_LOGI(TAG, "=== SpiLcdDisplay конструктор ===");
    ESP_LOGI(TAG, "Размер: %dx%d", width_, height_);
    ESP_LOGI(TAG, "Panel handle: %p", panel_);
    ESP_LOGI(TAG, "Panel IO handle: %p", panel_io_);

    // draw white
    ESP_LOGI(TAG, "Отрисовка белого экрана для проверки...");
    std::vector<uint16_t> buffer(width_, 0xFFFF);
    ESP_LOGI(TAG, "Размер буфера: %zu пикселей (ширина=%d)", buffer.size(), width_);
    
    esp_err_t draw_ret;
    for (int y = 0; y < height_; y++) {
        draw_ret = esp_lcd_panel_draw_bitmap(panel_, 0, y, width_, y + 1, buffer.data());
        if (draw_ret != ESP_OK && y == 0) {
            ESP_LOGE(TAG, "ОШИБКА отрисовки белого экрана на строке %d: %s (0x%x)", y, esp_err_to_name(draw_ret), draw_ret);
        }
    }
    ESP_LOGI(TAG, "Белый экран отрисован (%d строк)", height_);
    vTaskDelay(pdMS_TO_TICKS(100)); // Дать время на отрисовку
    ...
    ESP_LOGI(TAG, "Включение дисплея...");
    {
        ...
        if (__err == ESP_ERR_NOT_SUPPORTED) {
            ESP_LOGW(TAG, "Панель не поддерживает disp_on_off; предполагаем ON");
        } else if (__err != ESP_OK) {
            ESP_LOGE(TAG, "ОШИБКА включения дисплея: %s (0x%x)", esp_err_to_name(__err), __err);
        } else {
    ...
    ESP_LOGI(TAG, "Инициализация библиотеки LVGL...");
    lv_init();
    ESP_LOGI(TAG, "LVGL инициализирован");
    ...
    ESP_LOGI(TAG, "Инициализация LVGL порта...");
    lvgl_port_cfg_t port_cfg = ESP_LVGL_PORT_INIT_CONFIG();
    port_cfg.task_priority = 1;
#if CONFIG_SOC_CPU_CORES_NUM > 1
    port_cfg.task_affinity = 1;
    ESP_LOGI(TAG, "  Привязка к ядру: 1");
#endif
    ESP_LOGI(TAG, "  Приоритет задачи: %d", port_cfg.task_priority);
    
    esp_err_t port_ret = lvgl_port_init(&port_cfg);
    if (port_ret != ESP_OK) {
        ESP_LOGE(TAG, "ОШИБКА инициализации LVGL порта: %s (0x%x)", esp_err_to_name(port_ret), port_ret);
    } else {
        ESP_LOGI(TAG, "LVGL порт инициализирован");
    }
    ...
    if (display_ == nullptr) {
        ESP_LOGE(TAG, "КРИТИЧЕСКАЯ ОШИБКА: Не удалось добавить дисплей в LVGL!");
        ESP_LOGE(TAG, "  Проверьте: SPI соединение, питание дисплея, правильность пинов");
        return;
    } else {
        ESP_LOGI(TAG, "Дисплей успешно добавлен в LVGL, handle: %p", display_);
    }

    if (offset_x != 0 || offset_y != 0) {
        ESP_LOGI(TAG, "Установка смещения: offset_x=%d, offset_y=%d", offset_x, offset_y);
        lv_display_set_offset(display_, offset_x, offset_y);
    } else {
        ESP_LOGI(TAG, "Смещение не требуется (offset_x=0, offset_y=0)");
    }

    ESP_LOGI(TAG, "Настройка UI...");
    SetupUI();
    ESP_LOGI(TAG, "=== SpiLcdDisplay инициализация завершена ===");
}
...
#else
void LcdDisplay::SetupUI() {
    ESP_LOGI(TAG, "=== SetupUI начало ===");
    DisplayLockGuard lock(this);
    
    if (current_theme_ == nullptr) {
        ESP_LOGE(TAG, "ОШИБКА: current_theme_ == nullptr!");
        return;
    }
    
    LvglTheme* lvgl_theme = static_cast<LvglTheme*>(current_theme_);
    ESP_LOGI(TAG, "Тема найдена: %p", lvgl_theme);
    
    auto text_font = lvgl_theme->text_font()->font();
    auto icon_font = lvgl_theme->icon_font()->font();
    auto large_icon_font = lvgl_theme->large_icon_font()->font();
    ESP_LOGI(TAG, "Шрифты загружены: text=%p, icon=%p, large_icon=%p", text_font, icon_font, large_icon_font);

    auto screen = lv_screen_active();
    if (screen == nullptr) {
        ESP_LOGE(TAG, "ОШИБКА: screen == nullptr!");
        return;
    }
    ESP_LOGI(TAG, "Активный экран найден: %p", screen);
    lv_obj_set_style_text_font(screen, text_font, 0);
    lv_obj_set_style_text_color(screen, lvgl_theme->text_color(), 0);
    lv_obj_set_style_bg_color(screen, lvgl_theme->background_color(), 0);
    ...
    ESP_LOGI(TAG, "UI элементы созданы:");
    ESP_LOGI(TAG, "  container_: %p", container_);
    ESP_LOGI(TAG, "  emoji_box_: %p", emoji_box_);
    ESP_LOGI(TAG, "  emoji_label_: %p", emoji_label_);
    ESP_LOGI(TAG, "  emoji_image_: %p", emoji_image_);
    ESP_LOGI(TAG, "  preview_image_: %p", preview_image_);
    ESP_LOGI(TAG, "  top_bar_: %p", top_bar_);
    ESP_LOGI(TAG, "  status_bar_: %p", status_bar_);
    ESP_LOGI(TAG, "  content_: %p", content_);
    ESP_LOGI(TAG, "  bottom_bar_: %p", bottom_bar_);
    ESP_LOGI(TAG, "  chat_message_label_: %p", chat_message_label_);
    ESP_LOGI(TAG, "=== SetupUI завершена успешно ===");
}
