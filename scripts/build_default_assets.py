...
def get_custom_wake_word_model_paths(project_root):
    """
    Get the full paths to custom wake word model directories in models/custom_wake_word/
    Returns a list of valid model paths
    """
    custom_models_dir = os.path.join(project_root, "models", "custom_wake_word")
    valid_paths = []
    
    if not os.path.exists(custom_models_dir):
        return valid_paths
    
    # Look for model directories (each directory should contain a .bin file)
    for item in os.listdir(custom_models_dir):
        model_dir = os.path.join(custom_models_dir, item)
        if os.path.isdir(model_dir):
            # Check if directory contains model files (.bin)
            has_model = False
            ...
            if has_model:
                ...
                print(f"Found custom wake word model: {item}")
...
def main():
    ...
    # Calculate project root for custom models
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    ...
    custom_wake_word_paths = []
    
    # Check for custom wake word models first (they take priority)
    custom_wake_word_paths = get_custom_wake_word_model_paths(project_root)
    if custom_wake_word_paths:
        print(f"  Found {len(custom_wake_word_paths)} custom wake word model(s)")
        # Custom models are treated as wakenet models
        wakenet_model_paths.extend(custom_wake_word_paths)
    
    # 1. Only package wakenet models if USE_ESP_WAKE_WORD=y or USE_AFE_WAKE_WORD=y
    # Custom models take priority and work with ESP/AFE wake word types
    if custom_wake_word_paths:
        # Custom models found - they will be used (already added to wakenet_model_paths above)
        # Also add standard models if configured
        if wake_word_config['use_esp_wake_word'] or wake_word_config['use_afe_wake_word']:
            standard_models = get_wakenet_model_paths(wakenet_model_names, args.esp_sr_model_path)
            # Add standard models only if they don't conflict
            for std_model in standard_models:
                if std_model not in wakenet_model_paths:
                    wakenet_model_paths.append(std_model)
    elif wake_word_config['use_esp_wake_word'] or wake_word_config['use_afe_wake_word']:
        # No custom models, use standard models
    ...
    if wakenet_model_paths:
        if custom_wake_word_paths:
            custom_names = [os.path.basename(p) for p in custom_wake_word_paths]
            print(f"  custom wake word models: {', '.join(custom_names)} (will be packaged)")
        if wakenet_model_names:
